...
async function main(): Promise<void> {
  ...
  const checks: Array<{ url: string; expectStatus: number; label: string }> = [
    ...
    // 本地 news fixtures 已清空；未启 article 源时详情 404、列表为空。启 NEWS_ARTICLE 后详情/列表由管理端填充。
    { url: `${baseUrl}/api/news/missing-id`, expectStatus: 404, label: 'GET /api/news/missing-id' },
