...
export function getProductCategories(req: Request, res: Response): void {
  if (req.mockScenario === 'empty') {
    success(res, withAbsoluteAssets(req, { slides: [], categories: [] }), req.requestId);
    return;
  }
  success(res, withAbsoluteAssets(req, getFixtures().productCategories), req.requestId);
}
...
export function getProducts(req: Request, res: Response): void {
  ...
  const filtered = getFixtures().products.filter((item) => {
    if (category && item.category.id !== category) {
      return false;
    }
    return matchesKeyword(`${item.name} ${item.nameCn} ${item.summary}`, keyword);
  });
  success(res, withAbsoluteAssets(req, paginate(filtered, page, pageSize)), req.requestId);
}

export function getProductDetail(req: Request, res: Response): void {
  const detail = getFixtures().productDetails.find((item) => item.id === req.params.id);
  if (!detail) {
