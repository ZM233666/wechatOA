...
正式后端状态列当前均为「未实现」。路径与 DTO 冻结后由 NestJS 按同契约实现。
...
| GET | `/api/profile` | 我的 | `loggedIn=true` 可选（Mock-only） | `ProfileData` | `guest.json` / `logged-in.json` | 未实现 | 默认访客；`loggedIn` 不得进入 NestJS |
